"""
Original code is brought from https://github.com/abisee/cnn-dailymail.
Modified by Beurett to fit own data format.
"""
...
def write_to_bin(story_fnames, out_file, makevocab=False):
    ...
    print("Making bin file... =>%s" % out_file)
    ...
    with open(out_file, 'wb') as writer:
        for idx, s in enumerate(story_fnames):
            ...
            if os.path.isfile(os.path.join(tokenized_stories_dir, s)):
                ...
                raise Exception("Error: Couldn't find tokenized story file %s in story directory. Was there an error during tokenization?" % s)
...
if __name__ == '__main__':
    ...
    # Read the tokenized stories
    story_fnames = os.listdir(tokenized_stories_dir)

    # Randomize stories
    random.shuffle(story_fnames)

    # Split stories into 3 lists by each ratio [train:val:test] = [0.7:0.1:0.2]
    [fnames_train, fnames_val, fnames_test] = [
        story_fnames[:int(len(story_fnames) * 0.7)],
        story_fnames[int(len(story_fnames) * 0.7):int(len(story_fnames) * 0.9)],
        story_fnames[int(len(story_fnames) * 0.9):]
    ]
    
    # do a little postprocessing then write to bin files
    write_to_bin(fnames_test, os.path.join(finished_files_dir, "test.bin"))
    write_to_bin(fnames_val, os.path.join(finished_files_dir, "val.bin"))
    write_to_bin(fnames_train, os.path.join(finished_files_dir, "train.bin"), makevocab=True)
